"""
Примеры различных методов для работы окна поверх всех других окон.
...
try:
    ...
    logger.success("PySide6 доступен")
except ImportError:
...
try:
    ...
    logger.success("Windows API доступен")
except ImportError:
    WIN32_AVAILABLE = False
    logger.error("Windows API не найден")
...
class Example4_UltraAggressive(QWidget):
    ...
    def ultra_force_topmost(self):
        try:
            ...
            logger.error(f"Ошибка ультра-агрессивных методов: {e}")
...
class Example5_LayeredWindow(QWidget):
    ...
    def setup_layered_window(self):
        try:
            ...
            logger.error(f"Ошибка Layered Window: {e}")
        ...
        try:
            ...
            if hwnd:
                ...
                if not ctypes.windll.user32.IsWindowVisible(hwnd):
                    logger.warning("Окно не видимо, восстанавливаем...")
            ...
            logger.error(f"Ошибка проверки видимости: {e}")
...
def show_example(example_class, name):
    ...
    try:
        ...
        window.show()
        logger.success(f"Показан пример: {name}")
        return app, window
    except Exception as e:
        logger.error(f"Ошибка показа примера {name}: {e}")
        return None, None
...
def main():
    ...
    if not PYSIDE6_AVAILABLE:
        logger.error("PySide6 не доступен!")
        return
        
    logger.info("Тестирование методов работы поверх всех окон")
    logger.info("=" * 50)
    ...
    try:
        ...
        if choice == 0:
            ...
            if app and window:
                logger.info(f"Тестируйте пример '{name}' в играх")
