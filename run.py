...
from PySide6.QtCore import Qt, QTimer, Signal, QObject, QEvent, QThread, QCoreApplication, QLockFile, QDir
from PySide6.QtGui import QCursor, QColor, QAction, QPixmap, QPainter, QFont, QIcon
    ...
    def is_already_running(self):
        ...
        try:
            ...
            lock_path = os.path.join(
                tempfile.gettempdir(), f"{self.app_name}.lock")
                    ...
                    with open(lock_path, 'r') as f:
                        ...
                        if pid_str.isdigit():
                            ...
                            try:
                                ...
                                print(
                                    f"WARNING Приложение уже запущено (PID: {pid})")
                                return True
                            except OSError:
                                # Процесс не существует, удаляем старый файл блокировки
                                print(
                                    f"TOOL Удаляем старый файл блокировки (PID {pid} не существует)")
                                os.unlink(lock_path)
                except Exception:
...
class Win32HotkeyManager(QObject if PYSIDE6_AVAILABLE else object):
    ...
    def start(self):
        ...
        try:
            ...
            print(
                f"ERROR Ошибка запуска глобальных горячих клавиш (win32): {e}")
            self._running = False
            return False
    ...
    def _monitor_hotkeys(self):
        ...
        try:
            ...
            win32api.RegisterHotKey(
                self._hwnd, 1, win32con.MOD_CONTROL, ord('C'))
...
class KeyboardHotkeyManager(QObject):
    ...
    def start(self):
        ...
        try:
            ...
            print(
                f"ERROR Ошибка запуска глобальных горячих клавиш (keyboard): {e}")
            self._running = False
            return False
            ...
            try:
                if hasattr(keyboard, '_listener'):
                    ...
                    try:
                        ...
                        print(
                            "TOOL Принудительная инициализация keyboard выполнена успешно")
                    except Exception:
                        print(
                            "WARNING Keyboard listener не работает после инициализации")
                else:
                    print("WARNING Keyboard listener не существует после инициализации")
            except Exception as e:
                print(
                    f"WARNING Ошибка проверки keyboard после инициализации: {e}")
                ...
                if current_time - last_check > 2.0:
                    ...
                    try:
                        ...
                        if not hasattr(keyboard, '_listener'):
                            print(
                                "WARNING Keyboard listener не существует, перезапускаем...")
                            self._force_init_keyboard()
                        else:
                            # работоспособность через is_pressed
                            try:
                                ...
                                print(
                                    "WARNING Keyboard listener не работает, перезапускаем...")
                                self._force_init_keyboard()
                    except Exception as e:
...
class ClickableLabel(QLabel):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            self.setStyleSheet(
                original_style + STYLES['clickable_label_copied'])
...
class ColorPickerState(QObject):
    ...
    def update_live_data(self):
        ...
        try:
            ...
            changed = (self._coordinates != (x, y)) or (
...
class FixedDesktopColorPicker(QWidget if PYSIDE6_AVAILABLE else object):
    """
    Главное окно приложения. Отвечает за отображение (View).
    """

    def __init__(self):
        super().__init__()
        # Состояние приложения
        self.state = ColorPickerState(self)
        self.state.state_changed.connect(self.update_display)

        # Убираем стандартную рамку окна и делаем фон прозрачным
        self.setWindowFlags(Qt.FramelessWindowHint |
                            Qt.Tool | Qt.WindowStaysOnTopHint)
        self.setAttribute(Qt.WA_TranslucentBackground)
        self.setAttribute(Qt.WA_ShowWithoutActivating)
        ...
        self._hotkey_check_timer = QTimer(self)
        self._hotkey_check_timer.timeout.connect(
            self._check_hotkeys_periodically)
        self._hotkey_check_timer.start(5000)  # Каждые 5 секунд
        ...
        if I18N_AVAILABLE:
            try:
                ...
                print(
                    f"🌐 Язык инициализирован: {get_language_name(Language(saved_language))}")
            except Exception as e:
        ...
        self.hotkey_manager.ctrl_pressed.connect(self.state.toggle_freeze)
        ...
        self.coordinates_timer = QTimer(self)
        self.coordinates_timer.timeout.connect(self.state.update_live_data)
        self.coordinates_timer.start(16)  # ~60 FPS

        # Таймер для проверки видимости окна в играх
        self.visibility_timer = QTimer()
        self.visibility_timer.timeout.connect(
            self._safe_check_window_visibility)
        # Проверка каждые 500мс для стабильности
        self.visibility_timer.start(500)

        # Таймер для проверки и закрытия зависших меню
        self.menu_cleanup_timer = QTimer()
        self.menu_cleanup_timer.timeout.connect(
            self._check_and_close_stale_menus)
    ...
    def setup_ui(self):
        ...
        self.hotkey_status.setSizePolicy(
    ...
    def capture_color(self):
        ...
        try:
            ...
            print(
                f"Захвачен цвет: {hex_color} RGB({r}, {g}, {b}) в позиции ({x}, {y})")
...
def keyPressEvent(self, event):
    """Обработка нажатий клавиш, когда окно в фокусе."""
...
def contextMenuEvent(self, event):
    """Вызывается при правом клике на виджет."""
        ...
        if KEYBOARD_AVAILABLE:
            ...
            if hasattr(keyboard, '_listener'):
                ...
                try:
                    ...
                    print(
                        f"INFO Проверка keyboard: listener не работает - {e}")
                    return False
            else:
...
def _monitor_hotkeys_periodically(self):
    ...
    try:
        ...
        if (WIN32_AVAILABLE or KEYBOARD_AVAILABLE) and hasattr(self, '_hotkeys_initialized'):
            if not self._test_hotkeys_working():
                print(
                    "🔄 Периодическая проверка: горячие клавиши не работают, восстанавливаем...")
                self.hotkey_manager.restart()
    except Exception as e:
...
def _show_context_menu(self, pos):
    ...
    try:
        ...
        self._context_menu.setWindowFlags(
        ...
        if WIN32_AVAILABLE or KEYBOARD_AVAILABLE:
            ...
            restart_hotkeys_action.triggered.connect(
                self.hotkey_manager.restart)
...
def show_from_tray(self):
    ...
    try:
        ...
        print(
            "GAME Окно принудительно показано из системного трея с ультра-агрессивными методами")
    except Exception as e:
        ...
        if WIN32_AVAILABLE:
            try:
                ...
                if hwnd:
                    ...
                    logger.game(
                        "Применены рабочие методы Windows API + Layered Window")
            except Exception as api_error:
                logger.error(
                    f"Ошибка рабочих методов Windows API: {api_error}")
    ...
    try:
        if WIN32_AVAILABLE:
            ...
            if hwnd:
                ...
                print(
                    "GAME Применены максимально агрессивные методы восстановления")
        ...
        if WIN32_AVAILABLE:
            ...
            if hwnd:
                ...
                if current_hwnd != 0:
                    ...
                    print(
                        "GAME Окно принудительно поднято через расширенные методы")
    ...
    try:
        ...
        for text, opacity in opacity_values:
            ...
            action.triggered.connect(
                lambda checked, o=opacity: self._set_opacity(o))
...
def _show_language_menu(self):
    ...
    try:
        ...
        for lang in languages:
            ...
            action.triggered.connect(
                lambda checked, l=lang: self._set_language(l.value))
...
def _show_about(self):
    ...
    try:
        ...
        if I18N_AVAILABLE:
            ...
            author_text = get_text("author").format(
...
def _check_hotkeys_periodically(self):
    """Периодически проверяет, работают ли горячие клавиши."""
    try:
        if not HotkeyManager.is_keyboard_working():
            print(
                "🔄 Периодическая проверка: горячие клавиши не работают, восстанавливаем...")
            self.hotkey_manager.restart()
    except Exception as e:
...
def _setup_context_menu(self):
    """Создает или обновляет контекстное меню."""
    # ... existing code ...
